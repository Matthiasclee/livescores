...
def make_leaderboard_embed(data, data_source, division, location, tier, page, per_page, highlight_teams, image_data, image_name):
    ...
    for i, team in enumerate(leaderboard_data):
        ...
        if (i+1) % 15 == 0: # 15 teams per field
            x += 1
...
def make_advancement_embed(season, team_data, state_data, nationals_data, excluded_teams, state_ds, semis_ds):
    ...
    if nationals_data and team_data["tier"] == "Platinum" and team_data["division"] == "Open":
        if semis_rank <= 12 and semis_rank != 0:
            nats_advancement_text = f"Advances to nationals: **yes**\n*Rank {semis_rank} in Open Platinum*"
        else:
            nats_advancement_text = f"Advances to nationals: **no**\n*Rank {semis_rank} in Open Platinum*"
...
def make_help_embed():
    embed = discord.Embed(
        title = f"LiveScores Help",
        description = f"[Join our Discord server](https://discord.com/invite/ysxkPsGQtn) | [Learn more](https://github.com/Matthiasclee/livescores/blob/master/readme.md) | [Github](https://github.com/Matthiasclee/livescores)",
        color=0
    )
    ...
    embed.add_field(
            name="`/leaderboard [division] [tier] [location] [page] [teams per page] [teams to highlight] [image] [data source]`",
